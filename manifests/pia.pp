class io_metricbeat::pia (
  ...
  $pia_fields                = $io_metricbeat::pia_fields
) inherits io_metricbeat {
  notify { "Create Metricbeat monitors for PIA of ${service_name}-${host}": }

  file { "${monitor_location}/pia-${service_name}-${host}.yml" :
    ensure  => file,
    content => template('io_metricbeat/pia.yml.erb'),
